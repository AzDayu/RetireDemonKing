...
public class EquipmentChestResultPanelUI : UIBase
{
        ...
        Action<EquipmentModel> onConfirm)
    {
        ...
        _currentEquipButton.interactable =
            currentData != null && currentModel != null;

        _newEquipButton.interactable =
            newData != null && newModel != null;
    ...
    // ShowDropSelection 또는 ShowRingSelection 호출 후 사용
    public void AppendPowerSummary(string summary)
    {
        if (_guide == null || string.IsNullOrWhiteSpace(summary))
        {
            return;
        }
    ...
    private bool Initialize()
    {
        ...
        if (_panelRect == null ||
            ...
            _newCard == null ||
            _equipButton == null ||
            _equipButtonText == null)
        {
            Debug.LogError(
                "[EquipmentChestResultPanelUI] " +
                "결과창 프리팹 참조를 찾지 못했습니다."
            );
        ...
        if (_droppedRingCard == null ||
            _currentEquipButton == null ||
            _newEquipButton == null)
        {
            Debug.LogError(
                "[EquipmentChestResultPanelUI] " +
                "동적 선택 UI를 생성하지 못했습니다."
            );
        ...
        _droppedRingCard.Button.onClick.AddListener(
            () => SelectEquipment(_droppedRingCard.Model)
        );

        _currentEquipButton.onClick.AddListener(
            () => ConfirmEquipment(_currentCard.Model)
        );

        _newEquipButton.onClick.AddListener(
            () => ConfirmEquipment(_newCard.Model)
        );
    ...
    private EquipmentCardView CreateDroppedRingCard()
    {
        ...
        Transform textTransform =
            cardObject.transform.Find("OptionText");
        ...
        Vector2 anchorMax)
    {
        ...
        Transform textTransform =
            buttonObject.transform.Find("Text");
    ...
    private void SetSingleEquipmentLayout(bool useImmediateEquipButtons)
    {
        ...
        _currentEquipButton.gameObject.SetActive(
            useImmediateEquipButtons
        );

        _newEquipButton.gameObject.SetActive(
            useImmediateEquipButtons
        );

        _equipButton.gameObject.SetActive(
            !useImmediateEquipButtons
        );
    }
    ...
    private void ApplyResponsivePanelScale()
    {
        ...
        float widthScale =
            Mathf.Max(
                0f,
                viewport.rect.width - HorizontalSafePadding
            ) / panelWidth;

        float heightScale =
            Mathf.Max(
                0f,
                viewport.rect.height - VerticalSafePadding
            ) / panelHeight;
        ...
        EquipmentModel comparisonModel)
    {
        ...
        // 개별 능력치 차이는 같은 능력치 종류일 때만 표시한다.
        // 종합 전투력은 AppendPowerSummary에서 별도로 표시한다.
    ...
    private float CalculateStat(
        ...
        int level)
    {
        return EquipmentPowerUtility.CalculateEquipmentStat(
            equipmentData,
            level
        );
    }
